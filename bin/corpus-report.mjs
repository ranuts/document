...
if (bad.length) {
  lines.push('');
  lines.push('| file | open | edit | save | content (L2) | visual (L3) | asc errors | dialog |');
  lines.push('| --- | --- | --- | --- | --- | --- | --- | --- |');
  for (const r of bad) {
    const esc = (v) =>
      String(v ?? '')
        ...
        .slice(0, 120);
    lines.push(
      `| ${esc(basename(r.file))} | ${esc(r.open)} | ${esc(r.edit)} | ${esc(r.save)} | ${esc(r.content)} | ${esc(r.visual)} | ${esc(JSON.stringify(r.ascErrors))} | ${esc(r.fatalDialog)} |`,
    );
  }
}
const md = lines.join('\n');
