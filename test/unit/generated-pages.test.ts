...
describe('generated markdown pages', () => {
  ...
  it('renders every page for every locale it has a source for', () => {
    ...
    const homes = outputs.filter((o) => o.kind === 'home');
    const expected = PAGES.reduce((n: number, p: { sources: object }) => n + Object.keys(p.sources).length, 0);
    // One homepage per locale that has a home.json, plus every markdown page.
    expect(homes.length).toBeGreaterThanOrEqual(2);
    expect(outputs.length).toBe(expected + homes.length);
    for (const o of outputs) {
      ...
      // Three shapes: the homepage builds its own hero, a landing page leads
      // with a call to action and no "source" footnote, and a documentation
      // page is prose with code and tables (.doc).
      if (o.kind !== 'home') {
