...
import { X2TConverter, canStreamWasm, fetchWasmResponse, x2tInstantiateError } from '@ranuts/converter';
 ...
 *
 * The package used to have only the buffered path: fetch the module and hand
 * emscripten a 42 MB ArrayBuffer. That buffer is alive at the exact moment
 * WebAssembly asks the browser for x2t's own 283 MB heap, which is the moment
 * that fails on a machine short of memory (GitHub #144). The site's own loader
...
describe('loadScript picks the path that keeps the inflated module out of the peak', () => {
  ...
  it('answers emscripten synchronously and starts the download from the hook', async () => {
    ...
    expect(pending).toHaveBeenCalledWith(expect.stringContaining('x2t.wasm.br'));
  });
});
...
describe('fetchWasmResponse', () => {
  ...
  it('asks again after a 5xx and returns the good answer', async () => {
    ...
    await expect(drive(fetchWasmResponse('x2t.wasm.br'))).resolves.toBe(good);
  ...
  it('gives up after three tries and reports the status', async () => {
    ...
    await expect(drive(fetchWasmResponse('x2t.wasm.br'))).rejects.toThrow(/Failed to fetch x2t WASM at .*503/);
  ...
  it('does not ask again for a 404 -- a deployment fact, and retrying only delays the error', async () => {
    ...
    await expect(drive(fetchWasmResponse('x2t.wasm.br'))).rejects.toThrow(/Failed to fetch x2t WASM at .*404/);
