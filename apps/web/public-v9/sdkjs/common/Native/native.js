...
native_pattern_fill.prototype = { setTransform: function (transform) {} };
function native_gradient_fill() {}
native_gradient_fill.prototype = { addColorStop: function (offset, color) {} };
...
native_canvas.prototype = {
  getContext: function (type) {
    return type == '2d' ? new native_context2d(this) : null;
  },
...
_null_object.getBoundingClientRect = function () {
  return { left: 0, top: 0, right: 0, bottom: 0 };
};
document.createElement = function (type) {
  if (type && type.toLowerCase) if (type.toLowerCase() == 'canvas') return new native_canvas();
  return _null_object;
};
...
window.InitNativeObject = function () {
  ...
  window.devicePixelRatio = 1;
  if (window.native && window.native.GetDevicePixelRatio) window.devicePixelRatio = window.native.GetDevicePixelRatio();
};
if (undefined !== native) window.InitNativeObject();
function NativeCreateApi(options) {
  window.NATIVE_DOCUMENT_TYPE = window.native.GetEditorType();
  Api = null;
  var configApi = {};
  if (options && undefined !== options['translate']) configApi['translate'] = options['translate'];
  ...
  if (options && undefined !== options['locale']) Api.asc_setLocale(options['locale']);
}
...
var console = {
  log: function (param) {
    window.native && window.native.ConsoleLog(param);
  },
  time: function (param) {},
  timeEnd: function (param) {},
  warn: function () {},
  error: function () {
    window.native && window.native.ConsoleLog('[error]: ' + param);
  },
};
var performance = (window.performance = (function () {
  ...
  return {
    now: function () {
      return Date.now() - basePerformanceOffset;
    },
  };
})());
