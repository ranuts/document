...
function UTF8ArrayToString(heap, idx, maxBytesToRead) {
  ...
  var endPtr = idx;
  for (; heap[endPtr] && !(endPtr >= endIdx); ) ++endPtr;
...
function lengthBytesUTF8(str) {
  ...
  for (; i < str.length; ++i) {
    ...
    if (u <= 127) ++len;
    else if (u <= 2047) len = len + 2;
    else if (u <= 65535) len = len + 3;
    else len = len + 4;
  }
  return len;
}
...
function _strftime(s, maxsize, format, tm) {
  ...
  for (rule in EXPANSION_RULES_2)
    if (pattern.indexOf(rule) >= 0) pattern = pattern.replace(new RegExp(rule, 'g'), EXPANSION_RULES_2[rule](date));
...
var _Spellchecker_Malloc = (Module['_Spellchecker_Malloc'] = function () {
  return (_Spellchecker_Malloc = Module['_Spellchecker_Malloc'] = Module['asm']['p']).apply(null, arguments);
});
var _Spellchecker_Free = (Module['_Spellchecker_Free'] = function () {
  return (_Spellchecker_Free = Module['_Spellchecker_Free'] = Module['asm']['q']).apply(null, arguments);
});
var _Spellchecker_Create = (Module['_Spellchecker_Create'] = function () {
  return (_Spellchecker_Create = Module['_Spellchecker_Create'] = Module['asm']['r']).apply(null, arguments);
});
var _Spellchecker_Destroy = (Module['_Spellchecker_Destroy'] = function () {
  return (_Spellchecker_Destroy = Module['_Spellchecker_Destroy'] = Module['asm']['s']).apply(null, arguments);
});
var _Spellchecker_AddDictionary = (Module['_Spellchecker_AddDictionary'] = function () {
  return (_Spellchecker_AddDictionary = Module['_Spellchecker_AddDictionary'] = Module['asm']['t']).apply(
    null,
    arguments,
  );
});
var _Spellchecker_RemoveDicrionary = (Module['_Spellchecker_RemoveDicrionary'] = function () {
  return (_Spellchecker_RemoveDicrionary = Module['_Spellchecker_RemoveDicrionary'] = Module['asm']['u']).apply(
    null,
    arguments,
  );
});
var _Spellchecker_Load = (Module['_Spellchecker_Load'] = function () {
  return (_Spellchecker_Load = Module['_Spellchecker_Load'] = Module['asm']['v']).apply(null, arguments);
});
var _Spellchecker_Spell = (Module['_Spellchecker_Spell'] = function () {
  return (_Spellchecker_Spell = Module['_Spellchecker_Spell'] = Module['asm']['w']).apply(null, arguments);
});
var _Spellchecker_Suggest = (Module['_Spellchecker_Suggest'] = function () {
  return (_Spellchecker_Suggest = Module['_Spellchecker_Suggest'] = Module['asm']['x']).apply(null, arguments);
});
var _Spellchecker_RemoveEngine = (Module['_Spellchecker_RemoveEngine'] = function () {
  return (_Spellchecker_RemoveEngine = Module['_Spellchecker_RemoveEngine'] = Module['asm']['y']).apply(
    null,
    arguments,
  );
});
var _Spellchecker_TotalAllocatedMemory = (Module['_Spellchecker_TotalAllocatedMemory'] = function () {
  return (_Spellchecker_TotalAllocatedMemory = Module['_Spellchecker_TotalAllocatedMemory'] = Module['asm']['z']).apply(
    null,
    arguments,
  );
});
...
function onMessageEvent(data, port) {
  if (data.type == 'init') {
    if (self.spellchecker) return;
    self.spellchecker = new Spellchecker();
    self.spellchecker.languagesPath = data.dictionaries_path;
    var languages = data.languages;
    var i;
    for (i in languages) self.spellchecker.addDefaultLanguage(i, languages[i]);
    self.spellchecker.init();
    return;
  }
  if (!self.spellchecker) return;
  self.spellchecker.messages.push(data);
  if (port) self.spellchecker.ports.push(port);
  if (1 < self.spellchecker.messages.length) return;
...
function Dictionary() {
  ...
  this.load_file = function (src, params) {
    ...
    xhr.onload = function () {
      ...
      if (xhr.sender.status == 2) self.spellchecker.onLoadDictionary(this.sender);
    };
...
function Spellchecker() {
  ...
  this.messages = [];
  this.ports = [];
  this.tmpStrings = new ArrayBuffer(1e3);
  this.engine = 0;
  this.maxEngines = 3;
  ...
  this.init = function () {
    if (0 == this.engine && self.engineInit) this.engine = this.createEngine();
  };
  this.addDefaultLanguage = function (id, path) {
    this.languages['' + id] = { aff: path + '/' + path + '.aff', dic: path + '/' + path + '.dic' };
  };
  ...
  this.checkMessage = function () {
    if (0 == this.messages.length || !self.engineInit) return;
    var m = this.messages[0];
    var isReady = true;
    var indexLang = 0;
    var lenLangs = m.usrLang.length;
    for (; indexLang < lenLangs; indexLang++) {
      ...
      if (!readyLang) {
        ...
        this.languageQueue.push(lang_key);
        langToReady.load();
        isReady = false;
        break;
  ...
  this.allocString = function (string) {
    ...
    if (testLen > this.tmpStrings.byteLength) this.tmpStrings = new ArrayBuffer(testLen);
    var code = 0;
    var index = 0;
    var outputIndex = 0;
    ...
    for (; index < inputLen; ) {
      ...
      if (code >= 55296 && code <= 57343 && index < inputLen)
        code = 65536 + (((code & 1023) << 10) | (1023 & string.charCodeAt(index++)));
  ...
  this.sendAnswer = function (data) {
    if (self.spellchecker.ports.length == 0) self.postMessage(data);
    else {
