...
## 七、这份记录之后又变了什么（2026-08-22 当天）

这份文档写于一期落地当天，后面同一天又改了几轮，术语和结构都动过。
读到这里时以下述为准：

| 变化                                                                                      | 出处                                                                          |
| ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| **`?doc=` 改名 `?saved=`**（`doc` 会被读成 docx；且它说的是"本机保存的哪一篇"）           | PR #179                                                                       |
| **"本地历史"改称"本机保存的文档"**，`not on disk` 改称"未导出"                            | PR #175                                                                       |
| **恢复条的"Discard"改成"以后再说"**——它本来就不删除任何东西，旧文案在承诺一个没发生的动作 | PR #175                                                                       |
| **列表从表格改成列表视图**，宽度从 1152 收回站点的 720 读式列                             | PR #175                                                                       |
| **确认框从 `window.confirm` 换成 `r-modal`**                                              | PR #175                                                                       |
| **保存优先写回文档自己的文件**（记住 File System Access 句柄），下载变成兜底              | PR #179                                                                       |
| **快照间隔从写死 90s 改成按实测导出耗时自适应**                                           | PR #180，见[导出成本实测](./2026-08-22-export-cost-and-adaptive-snapshots.md) |

另外，第五节记的四个坑之外又添一个：**E2E 等快照的窗口必须小于间隔下限**。
45 秒的窗口是按 90 秒间隔选的，间隔下限降到 30 秒之后它又变回"无论分支在不在都绿"——
同一个陷阱在同一条 spec 上踩了两次，现在窗口是 25 秒。

## 八、没做的和下一步

- ~~**第 1 层（File System Access 句柄）没做**~~ —— **已做**（PR #179）：记住句柄，
  之后的保存静默写回同一个文件；任何异常回落到下载并忘掉句柄，那既是容错也是"另存为"的出口。
- ~~**导出成本仍未量化**~~ —— **已量**（PR #180）：桌面热导出 42~123 ms，首次约 1.2 s
  （加载 x2t）。间隔因此改成 `clamp(上次导出耗时 × 300, 30s, 180s)`。真实语料的 p50/p95
  仍未跑（需要本地语料）。
