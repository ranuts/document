...
## 追记 2：夜间公开语料首跑（Apache POI test-data，100 文件探针）

`.github/workflows/nightly-corpus.yml` 手动触发 `limit=100`：POI 三个目录共
发现 1265 个文件（已按名排除密码/损坏/宏样本），前 100 个（72 `.doc` +
28 `.docx`）**97/100 通过**，3 个发现均为 `.doc` 打开失败并正确浮出
`asc_onError -82`（`47950_lower.doc` / `47950_upper.doc` / `Bug51944.doc`，
POI 的老格式回归样本，疑似 Word 6/95）。跑道 19.4 分钟 / 2 worker。
矩阵新增短路：收到致命 asc_onError 立即判定打开失败，不再等满 180s。
后续夜间默认 300 个，逐步抬到全量。
