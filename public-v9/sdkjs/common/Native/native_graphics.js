...
CNativeGraphics.prototype.create = function (nativecontrol, width_px, height_px, width_mm, height_mm) {
  ...
  var native_object = nativecontrol;
  if (window['IS_NATIVE_EDITOR']) native_object = window['native']['GetResourcesForGraphics']();
...
CNativeGraphics.prototype.SetFont = function (font) {
  if (null == font) return;
  var flag = 0;
  if (font.Bold) flag = flag | 1;
  if (font.Italic) flag = flag | 2;
  if (font.Bold) flag = flag | 4;
  if (font.Italic) flag = flag | 8;
...
CNativeGraphics.prototype.SetTextPr = function (textPr, theme) {
  ...
  if (theme) {
    this.m_oGrFonts.Ascii.Name = theme.themeElements.fontScheme.checkFont(
      this.m_oTextPr.RFonts.Ascii ? this.m_oTextPr.RFonts.Ascii.Name : null,
    );
    this.m_oGrFonts.EastAsia.Name = theme.themeElements.fontScheme.checkFont(
      this.m_oTextPr.RFonts.EastAsia ? this.m_oTextPr.RFonts.EastAsia.Name : null,
    );
    this.m_oGrFonts.HAnsi.Name = theme.themeElements.fontScheme.checkFont(
      this.m_oTextPr.RFonts.HAnsi ? this.m_oTextPr.RFonts.HAnsi.Name : null,
    );
    this.m_oGrFonts.CS.Name = theme.themeElements.fontScheme.checkFont(
      this.m_oTextPr.RFonts.CS ? this.m_oTextPr.RFonts.CS.Name : null,
    );
    this.m_oGrFonts.Ascii.Index = -1;
    this.m_oGrFonts.EastAsia.Index = -1;
    this.m_oGrFonts.HAnsi.Index = -1;
    this.m_oGrFonts.CS.Index = -1;
  } else this.m_oGrFonts = this.m_oTextPr.RFonts;
};
...
CNativeGraphics.prototype.SetFontSlot = function (slot, fontSizeKoef) {
  ...
  if (undefined !== fontSizeKoef) _lastFont.FontSize *= fontSizeKoef;
...
CNativeGraphics.prototype.put_BrushGradient = function (gradFill, points, transparent) {
  ...
  var i = 0;
  for (; i < gradFill.colors.length; i++) {
